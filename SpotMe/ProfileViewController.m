...
- (void)viewDidLoad
{
    ...
    forZoomingIMage = [UIButton buttonWithType:UIButtonTypeCustom];
    [forZoomingIMage setFrame:CGRectMake(0, 0, 320, 480)];
    [forZoomingIMage addTarget:self action:@selector(ResizeImage) forControlEvents:UIControlEventTouchUpInside];
...
-(void)viewDidAppear:(BOOL)animated{
    ...
    if(isRoot)
    {
           self.navigationController.tabBarController.navigationController.visibleViewController.navigationItem.title=@"Profile";
...
-(void)getUserProfile{
    ...
    [appDelegate.webSer getUserProfile:[userDict valueForKey:@"user_id"]];
...
-(void)webServiceRequestFinished:(NSString *)responseStr ;{
   ...
   [MBProgressHUD hideHUDForView:self.view animated:YES];
    ...
    if ([status isEqualToString:@"1"] || [status isEqualToString:@"true"]) {
        ...
        NSLog(@"dicationary %@",[objectsDict valueForKeyPath:@"Result.workout_info"]);
        if(fieldsArray==nil)
            fieldsArray=[[NSMutableArray alloc] initWithArray:[objectsDict valueForKeyPath:@"Result.workout_info"]];
...
-(IBAction)MainPush:(id)sender
{
    ...
    mainViewForChartandImage.frame = CGRectMake(0, 350, mainViewForChartandImage.frame.size.width, mainViewForChartandImage.frame.size.height);
...
-(IBAction)forVideos
{
    VideoViewController *gridViewController = [[VideoViewController alloc] initWithNibName:@"VideoViewController" bundle:nil];
    [self.navigationController pushViewController:gridViewController animated:YES];
    mainViewForChartandImage.hidden = YES;
}

-(IBAction)ResizeImage
{
    if (ImageZoome) {
     ImageZoome = NO;
    ...
    [UIView animateWithDuration:1.5 delay:0 options:0 animations:^{
        ...
        imgBgView.transform = CGAffineTransformMakeScale(4,4);
        imgBgView.center = CGPointMake(140,210);
    } completion:^(BOOL finished) {
        
        [self.view addSubview:forZoomingIMage];
    }];
    }
    else
    {
        ...
        [forZoomingIMage removeFromSuperview];

        [UIView animateWithDuration:1.5 delay:0 options:0 animations:^{
        imgBgView.transform = CGAffineTransformMakeScale(1,1);
            imgBgView.center = CGPointMake(28,24);
        } completion:^(BOOL finished) {
